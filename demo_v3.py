...
def main_worker():
    ...
    from licauc.optimizers import Adam
    ...
    if args.loss == "CrossEntropy" or args.loss == "CE" or args.loss == "BCE":
        ...
        loss_fn = AUCMLoss()
        optimizer = Adam(net.parameters(), lr=args.lr, weight_decay=weight_decay_value)
...
def train(net, train_loader, test_loader, loss_fn, optimizer, epochs):
    ...
    for e in range(epochs):
        ...
        if epoch_auc > max_auc:
            ...
            torch.save(net.state_dict(), f"saved_model/{args.data}-v3")
